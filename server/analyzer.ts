...
export async function analyzeImageAndDescription(imageBase64: string, description?: string): Promise<{ category: string; severity: string; summary: string }> {
  ...
  try {
    ...
    console.error("Gemini API call failed, falling back to heuristic analyzer:", error);
    ...
    const summary = `Smart Fallback: Detected possible ${category} issue${description ? ` ('${description}')` : ''}. (AI inspection service is temporarily experiencing high demand, fallback mode activated).`;
